...
export default function CategoryPage({ params }: Props) {
  ...
  const images = portfolioData.gallery.filter((g: any) => g.category === slug);
            ...
            {images.map((img: any, i: number) => (
              <div key={i} className="rounded-2xl overflow-hidden shadow">
                <div className="relative aspect-[3/4] w-full">
                  <Image src={img.src} alt={img.alt || `${category.title}-${i}`} fill sizes="(max-width:640px) 100vw, (max-width:1024px) 50vw, 33vw" className="object-cover" />
                </div>
              </div>
            ))}
          </div>
